...
public class ShinseiWorldGen implements IWorldGenerator {
    ...
    public void generateSurface(World world, Random random, int x, int z) {
        ...
        if(ConfigValues.booBauxiteValue == true){this.addOreSpawn(ShinseiBlocks.oreBauxite, world, random, x, z, 16, 16, 4+random.nextInt(6), 25, 38, 100);}
        ...
        if(ConfigValues.booCopperValue == true){this.addOreSpawn(ShinseiBlocks.oreCopper, world, random, x, z, 16, 16, 4 + random.nextInt(8), 35, 0, 60);}
        this.addOreSpawn(ShinseiBlocks.oreGraphite, world, random, x, z, 16, 16, 4+random.nextInt(6), 25, 38, 100);
        if(ConfigValues.booLeadValue == true){this.addOreSpawn(ShinseiBlocks.oreLead, world, random, x, z, 16, 16, 4+random.nextInt(6), 25, 38, 100);}
